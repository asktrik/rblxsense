...
local s_workspace    = game:GetService("Workspace")
local s_stats        = game:GetService("Stats")
...
--do
	...
	local Converted = {
		["_rblxsense.lua"] = Instance.new("ScreenGui");
		["_tick"] = Instance.new("LocalScript");
		["_watermark bg"] = Instance.new("Frame");
		...
		["_gs_notif_lib"] = Instance.new("ModuleScript");
	}

	-- Properties:

	Converted["_rblxsense.lua"].IgnoreGuiInset = true
	Converted["_rblxsense.lua"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
	Converted["_rblxsense.lua"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	Converted["_rblxsense.lua"].Name = "rblxsense_".._internal_randomstr(32)
	Converted["_rblxsense.lua"].Parent = s_runservice:IsStudio() and localplayer:WaitForChild("PlayerGui") or gethui() or s_coregui

	Converted["_watermark"].Font = Enum.Font.RobotoMono
	Converted["_watermark"].RichText = true
	Converted["_watermark"].Text = "<font color=\"#31ff42\">rblxsense</font> | focat9123123 | 00:00:00"
	...
	Converted["_watermark"].Parent = Converted["_rblxsense.lua"]

	Converted["_sense"].Font = Enum.Font.RobotoMono
	Converted["_sense"].Text = "sense"
	Converted["_sense"].TextColor3 = Color3.fromRGB(49, 155, 255)
	...
	Converted["_notifs"].Parent = Converted["_rblxsense.lua"]
	...
	Converted["_game"].Text = "rblxsense"
	...
	Converted["_sense1"].Font = Enum.Font.RobotoMono
	Converted["_sense1"].Text = "sense"
	Converted["_sense1"].TextColor3 = Color3.fromRGB(49, 155, 255)
	Converted["_sense1"].TextSize = 18
	Converted["_sense1"].TextXAlignment = Enum.TextXAlignment.Left
	Converted["_sense1"].BackgroundColor3 = Color3.fromRGB(26.000000350177288, 26.000000350177288, 26.000000350177288)
	Converted["_sense1"].BackgroundTransparency = 1
	Converted["_sense1"].BorderColor3 = Color3.fromRGB(0, 0, 0)
	Converted["_sense1"].BorderSizePixel = 0
	Converted["_sense1"].Position = UDim2.new(0.511304975, -101, 0.22626403, -13)
	Converted["_sense1"].Size = UDim2.new(0, 171, 0, 26)
	Converted["_sense1"].Name = "sense"
	Converted["_sense1"].Parent = Converted["_Template"]
	...
	function notif_lib:MakeNotification(notif_table: table)
		local text = notif_table.Description or "hey nigger"
		...
		newNotif.Name = "rblxsense.lua - ".._internal_randomstr(8)
	...
	local function DWNDR_fake_script() -- Fake Script: StarterGui.idk.rblxsense.lua.tick
		local script = Instance.new("LocalScript")
		script.Name = "tick"
		script.Parent = Converted["_rblxsense.lua"]
		local req = require
		...
		local watermark_text = watermark_gui["watermark"]
		...
		env["_internal"]["update_watermark"] = function()
			...
			local ping = math.floor(s_stats.PerformanceStats.Ping:GetValue())
			watermark_text.Text = string.format("<font color=\"#31ff42\">rblxsense</font> | %s ms| %s", ping, time)
		end
		...
		print(string.format("[ rblxsense.lua ] [ %s ] tick service connected", env["_internal"]["get_time"]()))
	end
...
function Library:New(options: table)
	...
	do
		-- StarterGui.ui lib
		GUI["1"] = Instance.new("ScreenGui", s_runservice:IsStudio() and localplayer:WaitForChild("PlayerGui") or gethui() or s_coregui)
		GUI["1"]["IgnoreGuiInset"] = true;
		GUI["1"]["ScreenInsets"] = Enum.ScreenInsets.DeviceSafeInsets;
		GUI["1"]["Name"] = [[rblxsense.lua]];
		GUI["1"]["ZIndexBehavior"] = Enum.ZIndexBehavior.Sibling;


		-- StarterGui.ui lib.rblxsense window
		...
		GUI["2"]["Position"] = UDim2.fromOffset(
			(viewportsize.X / 2) - GUI["2"].Size.X.Offset / 2, 
			(viewportsize.Y / 2) - GUI["2"].Size.Y.Offset / 2
		)
		GUI["2"]["BorderColor3"] = Color3.fromRGB(100, 100, 100);
		GUI["2"]["Name"] = [[rblxsense window]];
	end
	
	--// Title bar (+ seperator)
	do
		-- StarterGui.ui lib.rblxsense window.sep
		...
		-- StarterGui.ui lib.rblxsense window.titlebar
		...
		--// starts with "rblxsense"?
		if string.sub(options["Name"], 1, 9) == "rblxsense" then
			--// Green "sense" in "rblxsense.lua"
		...
		-- StarterGui.ui lib.rblxsense window.titlebar.rblxsense.lua
		...
		GUI["3c"]["Text"] = typeof(options["Name"] == "string") and options["Name"] or "rblxsense.lua";
		GUI["3c"]["Name"] = "rblxsense_".._internal_randomstr(32)
		GUI["3c"]["Position"] = UDim2.new(0.03947, 0, 0.20455, 0);


		-- StarterGui.ui lib.rblxsense window.titlebar.ImageLabel (Close button)
	...
	--// Navigation
	do
		-- StarterGui.ui lib.rblxsense window.tab selector
		...
		-- StarterGui.ui lib.rblxsense window.tab selector.UIListLayout
	...
	function GUI:CreateTab(options: table)
		...
		--// Render
		do
			-- StarterGui.ui lib.rblxsense window.tab selector.tab button
			...
			-- StarterGui.ui lib.rblxsense window.tab selector.tab button.UICorner
			Tab["2b"] = Instance.new("UICorner", Tab["2a"]);
			Tab["2b"]["CornerRadius"] = UDim.new(0, 2);


			-- StarterGui.ui lib.rblxsense window.tab selector.tab button.UIStroke
			...
			-- StarterTab.ui lib.rblxsense window.tab
			...
			-- StarterTab.ui lib.rblxsense window.tab.content
			...
			-- StarterTab.ui lib.rblxsense window.tab.content.UIListLayout
		...
		function Tab:Button(options: table)
			...
			do
				------------- BUTTON
				-- StarterButton.ui lib.rblxsense window.tab.content.button
				...
				-- StarterButton.ui lib.rblxsense window.tab.content.button.click
				...
				-- StarterButton.ui lib.rblxsense window.tab.content.button.click.icon
				...
				-- StarterButton.ui lib.rblxsense window.tab.content.button.UICorner
				Button["b"] = Instance.new("UICorner", Button["8"]);
				Button["b"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterButton.ui lib.rblxsense window.tab.content.button.UIStroke
			...
			function Button:SetText(text: string)
				assert(text, string.format("[ rblxsense ] [ button_%s ] text is required", options["Name"]))
				assert(type(text) == "string", string.format("[ rblxsense ] [ button_%s ] text must be a string", options["Name"]))
			...
			function Button:SetCallback(fn)
				...
				if fn ~= nil then
					assert(typeof(fn) == "function", string.format("[ rblxsense ] [ button_%s ] callback must be either nil or a function", options.Name))
				end
				options["Callback"] = fn
			end
			...
			do
				...
				Button["9"].MouseButton1Down:Connect(function()
					...
					if not s then
						...
						warn(string.format("[ rblxsense.lua ] [ %s - %s ] Error on button callback: %s", options.Name, _internal_gettime(), r))
					end
				end)
		...
		function Tab:Label(options: table)
			...
			do
				------------- LABEL
				-- StarterLabel.ui lib.rblxsense window.tab.content.label
			...
			function Label:SetText(text: string)
				assert(text, string.format("[ rblxsense ] [ label_%s ] text is required", options["GUID"]))
				assert(type(text) == "string", string.format("[ rblxsense ] [ label_%s ] text must be a string", options["GUID"]))
		...
		function Tab:Slider(options: table)
			...
			do
				------------------- SLIDER
				-- StarterGui.ui lib.rblxsense window.tab.content.slider
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.click
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.click.value
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.UICorner
				Slider["28"] = Instance.new("UICorner", Slider["25"]);
				Slider["28"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterGui.ui lib.rblxsense window.tab.content.slider.UIStroke
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.SliderBG
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.SliderBG.UICorner
				Slider["2b"] = Instance.new("UICorner", Slider["2a"]);



				-- StarterGui.ui lib.rblxsense window.tab.content.slider.SliderBG.Slider
				...
				-- StarterGui.ui lib.rblxsense window.tab.content.slider.SliderBG.Slider.UICorner
			...
			function Slider:SetValue(v: number)
				...
				if v == nil then --// setting via mouse/drag
					...
					local percentage = math.clamp(
						(playermouse.X - Slider["2a"].AbsolutePosition.X) / Slider["2a"].AbsoluteSize.X, 0, 1
					)
					...
					assert(v >= options.Min and v <= options.Max, string.format("[ rblxsense ] [ slider_%s ] value must be between %s and %s", options.Name, options.Min, options.Max))
					assert(typeof(v) == "number", "[ rblxsense ] [ slider_"..options["Name"].." ] value must be a number")
		...
		function Tab:Toggle(options: table)
			...
			do
				------------- TOGGLE
				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle
				...
				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle.click
				...
				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle.click.toggle
				...
				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle.click.toggle.UICorner
				Toggle["1b"] = Instance.new("UICorner", Toggle["1a"]);
				Toggle["1b"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle.UICorner
				Toggle["1c"] = Instance.new("UICorner", Toggle["18"]);
				Toggle["1c"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterToggle.ui lib.rblxsense window.tab.content.toggle.UIStroke
		...
		function Tab:Textbox(options: table)
			...
			do
				------------- Textbox
				-- StarterTextbox.ui lib.rblxsense window.tab.content.Textbox
				...
				-- StarterTextbox.ui lib.rblxsense window.tab.content.Textbox.UICorner
				Textbox["13"] = Instance.new("UICorner", Textbox["12"]);
				Textbox["13"]["CornerRadius"] = UDim.new(0, 2);


				-- StarterTextbox.ui lib.rblxsense window.tab.content.Textbox.UIStroke
				...
				-- StarterTextbox.ui lib.rblxsense window.tab.content.Textbox.input
				...
				-- StarterTextbox.ui lib.rblxsense window.tab.content.Textbox.icon
			...
			function Textbox:SetValue(text: string)
				assert(text, string.format("[ rblxsense ] [ Textbox_%s ] text is required", options["Placeholder"]))
	...
	Library:Notify({
		Description = "rblxsense has been loaded.",
		Duration = 5
	})
	return GUI
end
